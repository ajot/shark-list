...
@bp.route('/bulk-add-members', methods=['GET', 'POST'])
def bulk_add_members():
    """Bulk add members by Twitter handles - creates pending submissions"""
    if request.method == 'GET':
        return render_template('admin/bulk_add.html')

    # POST - process the bulk addition
    handles_text = request.form.get('handles', '').strip()

    if not handles_text:
        flash('Please provide at least one Twitter handle', 'error')
        return render_template('admin/bulk_add.html')

    # Parse handles (split by newlines, strip whitespace, remove @ symbols)
    handles = []
    for line in handles_text.split('\n'):
    ...
    for handle in handles:
        ...
        existing_member = ListMember.query.filter_by(username=normalized_handle).first()
        if existing_member:
        ...
        # Check if already exists as a submission
        ...
        if existing_submission:
            if existing_submission.status == Submission.STATUS_PENDING:
                ...
                results['skipped'].append({
                    'handle': handle,
                    'reason': 'Already approved (awaiting sync)'
                })
                continue
            # If rejected, delete the old submission and allow re-submission
            else:
                try:
                    ...
                    results['failed'].append({
                        'handle': handle,
                        'error': f'Failed to delete old rejected submission: {str(e)}'
                    })
                    continue

        try:
            # Create pending submission
            # Use placeholder email for bulk submissions
            submission = Submission(
                email='bulk-added@system',
    ...
    message_parts = []
    if results['success']:
        message_parts.append(f"{len(results['success'])} submitted for approval")
    if results['skipped']:
    ...
    if message_parts:
        flash(', '.join(message_parts), 'success' if results['success'] else 'error')

    return render_template('admin/bulk_add.html', results=results)
